...
export const getLoopsStatusRequest = async (): Promise<DataApiResponseDto<{ status: string }>> => {
  return (await api.get(ApiRoutes.LOOPS_STATUS)) as DataApiResponseDto<{ status: string }>;
};
